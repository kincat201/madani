...
@section('pageTitle','Pengaturan FAQ')
        ...
        <div class="page-content">
            ...
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li><span>Pengaturan Bantuan & FAQ</span></li>
                </ul>
            </div>
            <!-- END PAGE HEADER-->
            <div class="row">
                    ...
                    <div class="portlet light bordered margin-top-20">
                        ...
                        <div class="portlet-body form">
                            <form action="javascript:;" id="formSetting" enctype="multipart/form-data">
                                ...
                                @php
                                    $i = 0;
                                    @$faq = json_decode($model->faq);
                                @endphp
                                @foreach($model::FAQ_FIELD as $keyField => $field)
                                    @if($i%2==0)
                                        <div class="row">
                                            @endif
                                            <div class="col-md-6">
                                                <div class="form-group" id="{{$keyField}}">
                                                    <label class="control-label">{{$model::FAQ_LABEL[$keyField]}}</label>
                                                    @if($field == 'image')
                                                        <input type="file" name="{{$keyField}}" class="form-control" value="{{@$faq->$keyField}}" placeholder="{{$model::FAQ_LABEL[$keyField]}}">
                                                        <br>
                                                    ...
                                                    @elseif($field != 'textarea')
                                                        <input type="{{$field}}" name="{{$keyField}}" class="form-control" value="{{@$faq->$keyField}}" placeholder="{{$model::FAQ_LABEL[$keyField]}}">
                                                    @else
                                                        <textarea class="form-control" name="{{$keyField}}" rows="8">{{@$faq->$keyField}}</textarea>
                                                    @endif
                                                    <div class="help-block">{{ in_array($keyField,$model::FAQ_LABEL_LIST)? $model::FAQ_LABEL_HELP[$keyField]:''}}</div>
